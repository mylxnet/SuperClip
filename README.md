...
1. 在 [Releases](https://github.com/your-username/SuperClip/releases) 下载 `SuperClip_v2.0.1_便携版.zip`
2. 解压到任意目录
...
产物：`bin\Release\netcoreapp3.1\win-x64\publish\SuperClip.exe`（单文件，约 130 MB），同时自动生成带版本号的分发副本 `SuperClip_v2.0.1.exe`（发布目标 `CopyVersionedExe` 自动复制；固定名 exe 保留，供安装脚本与进程名依赖使用）
...
Output: `bin\Release\netcoreapp3.1\win-x64\publish\SuperClip.exe` (~130 MB), plus a versioned distribution copy `SuperClip_v2.0.1.exe` (auto-generated by the `CopyVersionedExe` publish target).
